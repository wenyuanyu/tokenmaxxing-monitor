...
TokenMaxxing Monitor 是一个本地优先的 AI Coding Agent token 活动仪表盘。电脑端
Node.js bridge 汇总配置过的数据源，通过 BLE 推送到 Waveshare
ESP32-S3-RLCD-4.2，并把同一份状态镜像到 macOS 菜单栏 App。

固件烧录后有意保持小而离线。数据源适配集中在 host bridge 里，所以后续要支持
Codex、Qwen Code、Claude 或其他 Agent，通常只需要扩展一个 JavaScript reader，
而不是改嵌入式 UI。
...
Activity cell 使用 5 档单色纹理，因为 RLCD 实际上是 1-bit：
...
│   ├── package.json
│   └── src/index.js
├── firmware/               # ESP32-S3-RLCD-4.2 的 ESP-IDF 工程
│   ├── main/
│   └── components/
├── menubar/                # 可选 macOS SwiftUI 菜单栏 App
│   ├── QwenBridgeBar.swift
│   └── build.sh
...
配对时，聚焦菜单栏 App 里的 BLE Name 输入框。只有这个输入框保持焦点时，App
才会扫描附近匹配的设备；选择设备屏幕最下方显示的完整 BLE 名，例如
`QwenToken_XXXX`，再点击 **Apply**。输入框失焦或 Apply 后，候选框会停止扫描并
收起。Bridge 之后只会精确连接这个名字，不会误连工位附近的其他 TokenMaxxing
设备。
...
| `QWEN_BLE_DEVICE_NAME` | `QwenToken,Qwen Usage` | 可连接的完整 BLE 设备名，逗号分隔。默认固件请填写屏幕最下方显示的完整 `QwenToken_XXXX`。 |
...
| `QWEN_BLE_SCAN_STUCK_RESTART_MS` | `0` | 可选兜底：持续扫描这么久仍看不到设备时退出，让 LaunchAgent/launchd 恢复卡住的 macOS BLE 扫描；默认关闭，因为离开设备附近是正常状态 |
...
`updatedAtUnix` 是 Unix 秒。`tzOffsetMinutes` 是本地时区相对 UTC 的分钟偏移，
例如 UTC+8 为 `480`。固件用这两个字段计算本地 header 时钟，以及 activity 页面
自然周/月标签。
...
- 电池状态绘制成带边框的百分比。上报充电时，一个很小的 1 x 1-pixel-art 闪电
  字形会放在电池框右侧；未充电时，百分比会重新居中，避免右侧空出一块。
...
本地预览可以在不烧录硬件的情况下，把 LVGL UI 渲染成 framebuffer 文件。先至少
build 一次固件，让 `font_zh18.c` 被生成。
...
3. 填充：
   - today totals
   - model totals
   - session events，用于 active time
   - task events，用于 longest task
   - daily totals，用于 activity/lifetime/peak/streak
...
- 确认屏幕最下方显示的 BLE 名和 `QWEN_BLE_DEVICE_NAME` 完全一致。
- 确认设备已上电并在蓝牙范围内。
